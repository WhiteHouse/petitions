...
Drupal.behaviors.loginlogout = {
  // use js to add a destination parameter to the login/logout link
  attach: function(context) {
    var fullPath = location.href;
    var basePath = Drupal.settings.basePath;

    $('span.loginout a').each(function(index, Element) {
      $(Element).attr('href', function (index, attribute) {
        temppath = function(val) {
          if(val.indexOf("?destination=") != -1) {
            val = val.substr(0, val.indexOf("?destination="));
          }
          return val;
        }($(this).attr('href'));
          return temppath + '?destination=' + window.location.pathname.substring(1);
      });
    });
  ...
  attach: function(context) {
    ...
    $("#button-sign-this-petition").bind('click',
      function () {
        ...
        var left_margin = (maskWidth - 976) / 2;
        var left = pos.left + left_margin;
        $('#comment-mask').css({'width':maskWidth,'height':maskHeight,'top':'-'+pos.top+'px','left':'-'+left+'px'});
        $('#comment-mask').show();
